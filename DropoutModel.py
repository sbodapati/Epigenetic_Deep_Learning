...
p = float(sys.argv[1]) if len(sys.argv) > 1 else 0.0
print("Dropout p = " + str(p), file = sys.stderr)
ff_name = "DropoutReg_Training_p" + str(p) + "out.txt"
with open(ff_name, "w") as ff:
  print("opening Dropout Model_Training_out.txt", file = ff)
...
class Model(nn.Module):
    def __init__(self, input_size, inputLayerArray, p):
        ...
        self.dropout = nn.Dropout(p) # do we need to set dropout for each layer or just once?
...
class DataLoader():
    ...
    def __init__(self, type, batch_size=5000):
        if type == 'dev':
            self.dev = True
        if type == 'test':
            self.test = True
...
def run_training(num_epochs = 400, batch_size=5000, p = p):
    ...
    while train_data_loader.GetEpoch()<num_epochs:
        ...
        if train_data_loader.GetEpoch() != lastEpoch:
            ...
            dev_error = loss(Y_d_hat, Y_d).item()
            ...
            train_error_array[train_data_loader.GetEpoch() - 1] = train_error
            dev_error_array[train_data_loader.GetEpoch() - 1] = dev_error
            lastEpoch = train_data_loader.GetEpoch()
            # np.savetxt("errorVsEpoch_onFullDataset.csv", error_array, delimiter=",")
            torch.save(model.state_dict(), 'last_trained_Dropout_Model_p' + str(p))

        if train_data_loader.GetEpoch()%5 == 0:
            torch.save(model.state_dict(), './trainedModelCheckpoints/FullDatasetCheckpoint_DropoutModel_p' + str(p) + '_%d'%train_data_loader.GetEpoch())

    np.savetxt("errorVsEpoch_DropoutModel_p" + str(p) + "onFullDataset.csv", error_array, delimiter=",")
    torch.save(model.state_dict(), 'last_trained_DropooutModel_p' + str(p))
