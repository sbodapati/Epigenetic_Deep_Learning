...
with open(ff_name, "w") as ff:
    print("opening L1_reg_Training_out_with_lambda:%f.txt" %lambd, file = ff)
...
def run_L1_training(num_epochs = 300, batch_size=10000):
    ...
    while train_data_loader.GetEpoch()<num_epochs or abs(lastMSE-currentMSE)<10**-5:
        ...
        if train_data_loader.GetEpoch() != lastEpoch:
            ...
            lastEpoch = train_data_loader.GetEpoch()
            torch.save(model.state_dict(), 'last_trained_L1Reg_Model_lam:%f'%lambd)
            totalOverallMSE = 0
            numofbatches = 0

        if train_data_loader.GetEpoch()%5 == 0:
            torch.save(model.state_dict(), './trainedModelCheckpoints/L1_Reg_lam_%f_Checkpoint_%d'%(lambd, train_data_loader.GetEpoch()))
...
def main():
    ...
    x_1 = torch.rand(2,1,requires_grad=True)
    print(x_1)
    all_linear1_params = torch.cat([x.view(-1) for x in x_1])
    print(all_linear1_params)
    l2_reg = torch.norm(all_linear1_params, 2)
    print(l2_reg)
    l2_reg = torch.norm(all_linear1_params, 1)
    print(l2_reg)
